...
const Recipes: React.FC = () => {
  ...
  const handleStartBatch = (recipeName: string) => {
    setLocation(`/batches?recipe=${encodeURIComponent(recipeName)}`);
  };
            ...
            <div key={recipe.id} className="gemini-card rounded-4xl overflow-hidden group transition-all shadow-sm dark:bg-slate-800 dark:border-slate-700">
              <div className="p-8">
                ...
                <button 
                  ...
                  className="w-full flex items-center justify-center gap-2 py-3.5 bg-slate-50 dark:bg-slate-900 text-slate-600 dark:text-slate-400 rounded-full font-bold hover:bg-primary hover:text-white dark:hover:bg-primary dark:hover:text-white transition-all text-sm border border-slate-100 dark:border-slate-800 shadow-sm"
                >
          ...
          <div className="bg-white dark:bg-slate-800 w-full max-w-2xl rounded-[2.5rem] shadow-2xl overflow-hidden max-h-[90vh] flex flex-col border border-slate-200 dark:border-slate-700 animate-in zoom-in-95 duration-200 mx-4">
            <div className="px-10 pt-10 pb-6 border-b border-slate-50 dark:border-slate-700 flex justify-between items-start bg-white dark:bg-slate-800 shrink-0 relative">
              <div className="flex gap-5">
                <div className="w-16 h-16 rounded-3xl bg-primary-light dark:bg-primary/20 flex items-center justify-center text-primary dark:text-blue-400 shrink-0 shadow-sm">
            ...
            <div className="px-10 py-8 space-y-10 overflow-y-auto flex-1 custom-scrollbar bg-slate-50/20 dark:bg-slate-900/20">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {selectedRecipe.ingredients.map((ing, i) => (
                    <div key={i} className="flex items-center justify-between p-4 bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm">
                      ...
                      <span className="text-primary dark:text-blue-400 font-black text-sm">{ing.quantity} {ing.unit}</span>
                    </div>
                  ))}
                </div>
              </section>
              ...
              <section>
                ...
                <div className="space-y-4">
                  {selectedRecipe.steps && selectedRecipe.steps.map((proc, i) => (
            ...
            <div className="px-10 py-6 border-t border-slate-100 dark:border-slate-700 flex gap-4 shrink-0 bg-white dark:bg-slate-800">
              <button 
                onClick={() => setSelectedRecipe(null)}
                className="flex-1 py-4 bg-slate-50 dark:bg-slate-900 text-slate-500 dark:text-slate-400 rounded-2xl font-bold text-xs uppercase tracking-widest hover:bg-slate-100 dark:hover:bg-slate-700 transition-all shadow-sm"
              >
                Cerrar Detalle
              </button>
              <button 
                onClick={() => handleStartBatch(selectedRecipe.name)}
                className="flex-[2] py-4 bg-primary text-white rounded-2xl font-bold text-xs uppercase tracking-widest hover:bg-primary-dark transition-all flex items-center justify-center gap-2 shadow-lg shadow-primary/20"
              >
            ...
            <form onSubmit={handleSave} className="px-8 py-6 space-y-8 overflow-y-auto flex-1 custom-scrollbar">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <input required type="text" className="w-full bg-slate-50 dark:bg-slate-900 border-none rounded-2xl p-4 font-semibold text-slate-800 dark:text-white focus:ring-2 focus:ring-primary/20 outline-none transition-all placeholder:text-slate-300 dark:placeholder:text-slate-600 shadow-sm" value={name} onChange={e => setName(e.target.value)} placeholder="Ej: IPA Imperial" />
                </div>
                <div className="space-y-2">
                  ...
                  <input required type="text" className="w-full bg-slate-50 dark:bg-slate-900 border-none rounded-2xl p-4 font-semibold text-slate-800 dark:text-white focus:ring-2 focus:ring-primary/20 outline-none transition-all placeholder:text-slate-300 dark:placeholder:text-slate-600 shadow-sm" value={desc} onChange={e => setDesc(e.target.value)} placeholder="Ej: Perfil amargo y cítrico" />
                </div>
              </div>
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-center px-1">
                  ...
                  <button type="button" onClick={handleAddIngredient} className="text-primary dark:text-blue-400 text-[10px] font-bold uppercase hover:underline">
                    + Añadir
                  </button>
                </div>
                <div className="space-y-3">
                  {ingredients.map((ing, i) => (
                    <div key={i} className="flex gap-2 items-center animate-in slide-in-from-left-2 duration-200">
                      <input required placeholder="Nombre" type="text" className="flex-1 bg-slate-50 dark:bg-slate-900 border-none rounded-2xl p-4 text-sm font-semibold text-slate-800 dark:text-white shadow-sm" value={ing.name} onChange={e => {
                        const newIngs = [...ingredients];
                        newIngs[i].name = e.target.value;
                        setIngredients(newIngs);
                      }} />
                      <input required placeholder="Cant." type="text" className="w-20 bg-slate-50 dark:bg-slate-900 border-none rounded-2xl p-4 text-sm font-semibold text-slate-800 dark:text-white text-center shadow-sm" value={ing.quantity} onChange={e => {
                        const newIngs = [...ingredients];
                        newIngs[i].quantity = e.target.value;
                        setIngredients(newIngs);
                      }} />
                      <button type="button" onClick={() => handleRemoveIngredient(i)} className="p-4 text-rose-400 hover:bg-rose-50 dark:hover:bg-rose-900/20 rounded-2xl transition-all">
              ...
              <button type="submit" className="w-full bg-primary text-white py-4 rounded-2xl font-bold hover:bg-primary-dark transition-all mt-4 text-base shadow-lg shadow-primary/20">
                Guardar Receta
              </button>
            </form>
          </div>
